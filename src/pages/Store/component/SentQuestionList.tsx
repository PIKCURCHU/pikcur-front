...
const SentQuestionList:React.FC<{storeId:number}> = ({storeId}) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(1);
    const navigate = useNavigate();

    const [sentQuestionList, setSentQuestionList] = useState<QuestionItem[]>([]);

    const formatQnaList = sentQuestionList.map((q, index) => ({
    ...
    useEffect(() => {
        if (!storeId) return;
        api.get(`/store/${storeId}/questions/sent`, {
            currentPage
        })
            .then((res) => {
                console.log(res);
                setSentQuestionList(res.qnaList);
    ...
    return (
        <div style={{
            ...
            <PaginationButtons
                maxPage={totalPages} 
                page={currentPage} // 남긴 문의 페이지 상태 사용
                onChange={handlePageChange} // 남긴 문의 핸들러 사용
            />
        </div>
    );
}

export default SentQuestionList;