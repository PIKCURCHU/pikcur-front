...
const FindPassword: React.FC<FindPasswordProps> = () => {
    ...
    const nextButtonClickHandler = () => {
        api.post("/auth/members/find-id", { email })
            .then((res) => {
                navigate('/findPasswordSuccess', { state: { id: res.data } });
            })
            .catch((err) => {
                const errorData = err.response?.data;
                if (errorData) {
        ...
        <AuthLayout
            ...
            content={
                <div style={{ height: '346px', display: 'flex', flexDirection: 'column', gap: '30px', alignItems: 'center' }}>
                    <div>
                        <CustomInput width={326} height={56} placeholder="이메일" value={email} onChange={onEmailChange}/>
                        <Button
